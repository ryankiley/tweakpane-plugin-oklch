...
class OklchColor {
    ...
    displayedOklch() {
        ...
        const rounded = MODE_CHANNELS[mode].map((ch, i) => Number(vals[i].toFixed(digitsFor(ch.step))) / ch.scale);
        const k = convert([rounded[0], rounded[1], rounded[2]], modeSpaceId(mode), 'oklch');
    ...
    gamutLabel() {
        ...
        if (inGamut(oklch, 'oklch', 'srgb')) {
            return 'sRGB';
        }
        if (inGamut(oklch, 'oklch', 'p3')) {
            return 'P3';
        }
        return 'wide';
    }
...
class PickerController {
    ...
    constructor(doc, config) {
        ...
        const syncAlphaRow = () => {
            ...
            if (wants) {
                root.appendChild(this.alphaRow_); // always last, after the texts row
            }
            else {
